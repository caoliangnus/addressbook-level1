...
public class AddressBook {
    ...
    private static final String COMMAND_SORT_DESC = "Sort the program in alphabetical order";
    ...
    * For example, a person's name is stored as name:value in the HashMap
    * */
...
//    private static final String PERSON_PROPERTY_PHONE = "phone";
    ...
    private static void sortAccendingByName() {
        Collections.sort(ALL_PERSONS, new Comparator<HashMap<PersonProperty, String>>() {
            ...
            public int compare(HashMap<PersonProperty, String> person1, HashMap<PersonProperty, String> person2) {
                String person1Name = person1.get(PersonProperty.NAME);
                String person2Name = person2.get(PersonProperty.NAME);
    ...
    private static String executeEditPerson(String commandArgs) {
        ...
        final HashMap<PersonProperty, String> targetInModel = getPersonByLastVisibleIndex(targetVisibleIndex);
        final String extractedPersonInfo = extractPersonInfoFromEditPersonArgs(commandArgs.trim());
        editPersonFromAddressBook(targetInModel, extractedPersonInfo);
        return  getMessageForSuccessfulEdited(targetInModel);

    }

    private static boolean isEditPersonArgsValid(String rawArgs) {
        try {
            final int extractedIndex = extractTargetIndexFromEditPersonArgs(rawArgs.trim());
            final String extractedPersonInfo = extractPersonInfoFromEditPersonArgs(rawArgs.trim());
            return extractedIndex >= DISPLAYED_INDEX_OFFSET && isPersonDataExtractableFrom(extractedPersonInfo);
        } catch (NumberFormatException nfe) {
            return false;
        }
    }
    ...
    private static void editPersonFromAddressBook(HashMap<PersonProperty, String> exactPerson, String updatedPersonInfo) {
        String name = extractNameFromPersonString(updatedPersonInfo);
        String phone = extractPhoneFromPersonString(updatedPersonInfo);
        String email = extractEmailFromPersonString(updatedPersonInfo);
        exactPerson.put(PersonProperty.NAME, name);
        exactPerson.put(PersonProperty.PHONE, phone);
        exactPerson.put(PersonProperty.EMAIL, email);
        savePersonsToFile(getAllPersonsInAddressBook(), storageFilePath);
    }


    private static String getMessageForSuccessfulEdited(HashMap<PersonProperty, String> editedPerson) {
